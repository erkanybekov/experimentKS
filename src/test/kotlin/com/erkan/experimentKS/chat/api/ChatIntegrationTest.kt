...
class ChatIntegrationTest : AbstractIntegrationTest() {
	...
	@Test
	fun `chat room membership message history and idempotent send work`() {
		...
		org.junit.jupiter.api.Assertions.assertEquals(firstMessage.id, secondAttempt.id)
		...
		mockMvc.perform(
			...
			.andExpect(jsonPath("$.items[0].id").value(replyMessage.id.toString()))
			.andExpect(jsonPath("$.items[1].id").value(firstMessage.id.toString()))
